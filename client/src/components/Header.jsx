...
function Header() {
  ...
  useEffect(() => {
    ...
    if (path.startsWith("/publication-check/results/"))
      title = "Publication Check Results";
    else if (path === "/publication-check/results")
      title = "Publication Check Results";
    else if (path.startsWith("/publication-check")) title = "Publication Check";
  ...
  const handleLogoutClick = async () => {
    try {
      ...
      // ignore logout side-effect failures
    }
  };
  ...
  return (
    <header className="bg-[#000054] h-18 relative overflow-visible">
      <nav
        className="mx-auto flex flex-col md:flex-row w-full md:max-w-7xl h-full px-0 md:px-0 relative"
        aria-label="Global"
      >
        <div className="flex flex-col md:flex-row items-center justify-between w-full h-full overflow-visible relative px-4 md:px-0">
          ...
          <div className="flex items-center justify-between w-full md:w-auto mb-0 md:mb-0 h-full md:h-full">
            ...
            <div className="flex items-center md:hidden">
              ...
              <div className="relative mr-3 z-50" ref={mobileMenuRef}>
                ...
                {user && showDropdown && (
                  <div className="absolute right-0 top-full mt-6 w-44 bg-white shadow-lg shadow-gray-400 pt-3 border border-gray-200 flex flex-col items-start z-9999">
                    <span className="w-full text-black text-base px-4">
                      {user?.name || "User"}
                    </span>
                    <span className="w-full mb-1 text-[#6A6A6A] text-xs px-4 wrap-break-word">
              ...
              >
                <span
                  className={`block w-6 h-0.5 bg-white transition-all duration-300 ease-in-out absolute ${showMobileMenu ? "rotate-45" : "-translate-y-1.5"}`}
                ></span>
                <span
                  className={`block w-6 h-0.5 bg-white transition-all duration-300 ease-in-out ${showMobileMenu ? "opacity-0" : "opacity-100"}`}
                ></span>
                <span
                  className={`block w-6 h-0.5 bg-white transition-all duration-300 ease-in-out absolute ${showMobileMenu ? "-rotate-45" : "translate-y-1.5"}`}
                ></span>
          ...
          {/* Navigation links */}
          <div
            className={`md:flex md:flex-row md:items-center md:w-max md:h-full md:justify-end text-base md:text-lg md:relative ${showMobileMenu ? "flex flex-col absolute top-full left-0 right-0 w-full bg-[#000054] shadow-lg border-t border-gray-600 z-9998 min-h-max" : "hidden md:flex"}`}
          >
            <div
              className={`flex items-center justify-center px-0 md:px-8 h-12 md:h-full w-full md:w-auto ${path.startsWith("/publication-check") ? "bg-white" : "hover:bg-[#000032]"}`}
            >
              <a
                href="/publication-check"
                className="w-full md:w-auto text-center py-3 md:py-0"
              >
                <p
                  className={`font-medium hover:underline ${path.startsWith("/publication-check") ? "text-[#000054] font-semibold underline" : "text-white"}`}
                >
                  Publication Check
                </p>
              </a>
            ...
            <div
              className={`flex items-center justify-center px-0 md:px-8 h-12 md:h-full w-full md:w-auto ${path.startsWith("/search") ? "bg-white" : "hover:bg-[#000032]"}`}
            >
              <a
                href="/search"
                className="w-full md:w-auto text-center py-3 md:py-0"
              >
                <p
                  className={`font-medium hover:underline ${path.startsWith("/search") ? "text-[#000054] font-semibold underline" : "text-white"}`}
                >
                  Search Tool
                </p>
              </a>
            ...
            {loading && hadSession ? (
              ...
              >
                <span
                  className={`${path === "/saved-researchers" ? "text-[#000054]" : "text-white"} opacity-0 py-3 md:py-0`}
                >
                  Saved Profiles
                </span>
              </div>
              ...
              >
                <a
                  href="/saved-researchers"
                  aria-label="Saved Profiles"
                  className="w-full md:w-auto text-center py-3 md:py-0"
                >
                  <p
                    className={`font-medium hover:underline ${path === "/saved-researchers" ? "text-[#000054] font-semibold underline" : "text-white"}`}
                  >
                    Saved Profiles
                  </p>
                </a>
            ...
            <div
              className="hidden md:flex ml-0 md:ml-3 mr-5 relative mt-4 md:mt-0 mb-4 md:mb-0 w-full md:w-auto justify-center md:justify-end z-50"
              ref={menuRef}
            >
              <button
              ...
              {user && showDropdown && (
                <div className="absolute right-2 md:right-0 top-full mt-2 w-44 md:w-50 bg-white shadow-lg shadow-gray-400 pt-3 border border-gray-200 flex flex-col items-start z-9999">
                  <span className="w-full text-black text-base md:text-lg px-4">
                    {user?.name || "User"}
                  </span>
                  <span className="w-full mb-1 text-[#6A6A6A] text-xs md:text-sm px-4 wrap-break-word">
                    {user?.email || "Email address"}
                  </span>
                  <hr className="w-full border-t border-gray-300 mt-2" />
                  <div className="w-full flex px-4 py-3 items-center hover:bg-gray-200 cursor-pointer">
