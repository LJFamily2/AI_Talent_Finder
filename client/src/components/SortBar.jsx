...
export default function SortBar({ sortBy = 'match', sortOrder = 'desc', onChange }) {
  ...
  const baseBtn = 'px-3 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm border transition-colors flex items-center justify-center gap-2 whitespace-nowrap';
  const onClass = 'bg-[#E60028] text-white border-[#E60028] shadow-sm';
  ...
  return (
    <div className='w-full flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4'>
      <span className='text-sm sm:text-md text-[#6A6A6A] shrink-0'>Sort by</span>
      <div className='w-full sm:flex-1 overflow-x-auto sm:overflow-visible pb-1 sm:pb-0'>
        <div className='inline-flex sm:grid sm:grid-cols-6 gap-2 min-w-max sm:min-w-0'>
          ...
          <button type='button' className={`${baseBtn} ${active('name') ? onClass : offClass}`} onClick={() => cycle('name')} aria-pressed={active('name')}>
            ...
            <span className='text-[10px] sm:text-xs leading-none'>{arrow('name')}</span>
          </button>
          <button type='button' className={`${baseBtn} ${active('h_index') ? onClass : offClass}`} onClick={() => cycle('h_index')} aria-pressed={active('h_index')}>
            ...
            <span className='text-[10px] sm:text-xs leading-none'>{arrow('h_index')}</span>
          </button>
          <button type='button' className={`${baseBtn} ${active('i10_index') ? onClass : offClass}`} onClick={() => cycle('i10_index')} aria-pressed={active('i10_index')}>
            ...
            <span className='text-[10px] sm:text-xs leading-none'>{arrow('i10_index')}</span>
          </button>
          <button type='button' className={`${baseBtn} ${active('citations') ? onClass : offClass}`} onClick={() => cycle('citations')} aria-pressed={active('citations')}>
            ...
            <span className='text-[10px] sm:text-xs leading-none'>{arrow('citations')}</span>
          </button>
          <button type='button' className={`${baseBtn} ${active('works') ? onClass : offClass}`} onClick={() => cycle('works')} aria-pressed={active('works')}>
            ...
            <span className='text-[10px] sm:text-xs leading-none'>{arrow('works')}</span>
          </button>
        </div>
      </div>
    </div>
