...
function LandingPage() {
  ...
  // Intro effect: fullscreen video that shrinks to hero on scroll (first visit only)
  ...
  useEffect(() => {
    ...
    const headerEl = document.querySelector("[data-intro-header]");

    const update = () => {
      const scrollY = scrollYRef.current;
      const vh = window.innerHeight;
      // progress: 0 (no scroll) → 1 (scrolled one full viewport height)
      const progress = Math.min(scrollY / vh, 1);

      if (introOverlayRef.current) {
        // Interpolate: from 100vw/100vh fixed fullscreen → hero rect (full width, 100dvh from top:0)
        // We keep width at 100% always; shrink height from 100vh to 100dvh (same), but shift down to account for header
        const headerHeight = headerEl ? headerEl.offsetHeight : 64;
        // top: 0 → headerHeight
        const top = progress * headerHeight;
        // height: 100vh → 100vh - headerHeight
        const height = vh - progress * headerHeight;
        // border-radius: 0 → 12px
        const radius = progress * 12;

        introOverlayRef.current.style.top = `${top}px`;
        introOverlayRef.current.style.height = `${height}px`;
        introOverlayRef.current.style.borderRadius = `0 0 ${radius}px ${radius}px`;
        introOverlayRef.current.style.opacity = "1";
      }

      if (headerEl) {
        headerEl.style.opacity = String(progress);
        headerEl.style.pointerEvents = progress > 0.1 ? "auto" : "none";
      }
  ...
  return (
    <div className="w-full min-h-screen relative">
      <div
        className="w-full bg-[#000054] fixed top-0 left-0 z-10"
        data-intro-header=""
      >
        <Header />
      </div>

      {/* Fullscreen intro overlay — only rendered on first visit until scroll completes */}
      {!introComplete && (
        <div
          ref={introOverlayRef}
          style={{
            ...
            zIndex: 50,
            overflow: "hidden",
            willChange: "top, height, border-radius",
          }}
        >
          <video
            src={autoplayVideo}
          ...
          >
            ...
            >
              The world is big,
              <br />
              we bring talents closer to you
            </h1>
            <p style={{ fontSize: "clamp(0.8rem, 2vw, 1rem)", opacity: 0.7 }}>
      ...
      {/* Spacer so the user can scroll to trigger the intro shrink (2× viewport = scroll range) */}
      {!introComplete && (
        <div
      ...
      <hr
        id="hr-features"
        className="w-2/3 border-[#E60028] border-2 my-10 sm:my-15 mx-auto"
      />

      <h2 className="text-center font-bold text-2xl sm:text-3xl text-[#000054] mb-10 mt-10 sm:mt-20 px-4">
        {" "}
        What can we provide?
      </h2>
      ...
      <div className="w-full md:w-5/6 sm:hover:scale-100 h-max bg-[#000054] mx-auto rounded-2xl flex flex-col md:flex-row pt-10 border-1 border-[#000054] hover:scale-105 transition-transform duration-300 shadow-md px-4 md:px-0">
        <div className="w-full md:w-1/2 text-white pl-0 md:pl-10 flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-2xl md:text-3xl mb-5">
              Academics around the world,<br></br>now in one place
            </h3>
            <p className="text-justify text-base md:line-clamp-3">
              Pulling data from reliable databases such as Scopus, ORCID and
              OpenAlex, our product presents to you a list of potential
              academics with the necessary details already given, sparing you
              much time of looking up on various individual platforms.
            </p>
          </div>
        ...
        <div className="max-w-500 w-full md:w-2/5 px-0 md:px-8 mt-8 md:mt-0">
          <img
            src={imageFindAcademics}
            alt="Find Academics"
            className="border-gray-200 border-x-1 border-t-1 w-full"
          />
        </div>
      </div>
        ...
        <div className="w-full md:w-1/2 sm:hover:scale-100 h-full bg-[#000054] rounded-2xl pt-10 shadow-md hover:scale-105 transition-transform duration-300 shadow-md mb-8 md:mb-0 px-4 sm:px-5">
          <div className="h-max px-0 md:px-5">
            <h3 className="font-bold text-2xl md:text-3xl mb-4">
              Verify candidates’ CVs
            </h3>
            <p className="text-balance text-base md:line-clamp-3">
              By comparing candidates’ statements in CV with some of our
              Internet sources, this product could give early alarms to the
              recruiters, highlighting which claims of the candidates are
              unverifiable.
            </p>
            <button className="w-max bg-[#E60028] text-white px-6 md:px-8 py-3 mt-10 rounded-2xl mb-6 hover:underline text-base md:text-lg">
              <a href="/publication-check">Explore now</a>
            </button>
          </div>
          <div className="bg-white w-full h-1/2 py-8 px-0 md:px-5 border-1 rounded-b-lg border-gray-200 border-1">
            <img
              src={imageVerifyCV}
              alt="Verify CV"
              className="border-white border-x-1 border-t-1 w-full"
            />
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 sm:hover:scale-100 bg-[#000054] rounded-2xl shadow-md grow flex flex-col justify-between hover:scale-105 transition-transform duration-300 shadow-md px-4 sm:px-5">
          <div className="h-full px-0 md:px-5 pt-10 flex flex-col justify-between">
            <div>
              ...
              <p className="text-balance text-base md:text-lg sm:mb-5">
                Recruiters can export their search results for further analysis.
              </p>
            </div>
            <button className="w-max bg-[#E60028] text-white px-6 md:px-8 py-3 mb-6 rounded-2xl hover:underline text-base md:text-lg">
          ...
          <div className="bg-white w-full py-8 px-0 md:px-5 rounded-b-lg border-gray-200 border-1 flex flex-col md:flex-row justify-around gap-4 md:gap-0">
            <div className="w-full md:w-1/2 border-gray-300 border-1 mb-4 md:mb-0">
              <img
                src={imageExportToReport}
                alt="Sample CV"
                className="blur-xs w-full"
              />
            </div>
            <div className="flex flex-col items-center md:items-start">
              <button className="w-max bg-[#FFF2D3] text-black px-8 md:px-10 py-3 mt-4 rounded-full flex text-base md:text-lg">
                <img
                  src={fileIcon}
                  className="w-auto h-3 self-center mr-2"
                ></img>
                Export result
              </button>
              <img
                src={clickIcon}
                className="w-14 md:w-16 h-auto justify-self-end relative bottom-3"
                alt="Click Icon"
              ></img>
            </div>
          </div>
      ...
      <div className=" h-max bg-[#F3F4F5] text-[#000054] text-lg sm:text-xl text-center py-10 sm:py-15 px-4">
        <p className="mb-6 sm:mb-8">
          We extract the <b>essence</b> of multiple large databases{" "}
          <b>used by millions</b> around the world
        </p>
        <div className="w-full sm:w-3/4 flex flex-wrap justify-center sm:justify-between items-center mt-2 mx-auto gap-6 sm:gap-4 md:gap-0">
          <img
            src={scopusLogo}
            alt="Scopus Logo"
            className="h-12 sm:h-16 md:h-24 mx-2 md:mx-4 inline-block w-auto"
          />
          <img
            src={orcidLogo}
            alt="ORCID Logo"
            className="h-10 sm:h-12 md:h-16 mx-2 md:mx-4 inline-block w-auto"
          />
          <img
            src={openAlexLogo}
            alt="OpenAlex Logo"
            className="h-6 sm:h-8 md:h-10 mx-2 md:mx-4 inline-block w-auto"
          />
          <img
            src={googleScholarLogo}
            alt="Google Scholar Logo"
            className="h-14 sm:h-16 md:h-24 mx-2 md:mx-4 inline-block w-auto"
          />
        </div>
      ...
      <div className="w-full h-max text-[#000054] text-center py-10 sm:py-20 px-4">
        <h3 className="text-2xl sm:text-3xl mb-8 sm:mb-13 px-2">
          {" "}
          We strive for <b>high quality</b> <br></br>in all aspects of our
          product
        </h3>
        <div className="w-full sm:w-5/6 lg:w-2/3 bg-[#000054] text-white mx-auto py-8 sm:py-15 px-4 sm:px-8 lg:px-20 rounded-2xl flex flex-col sm:flex-row gap-6 sm:gap-10">
          <div className="w-full sm:w-1/3 flex flex-col items-center text-center">
            ...
            <p className="text-base sm:text-lg">
              Our product has undergone careful review and UX test before being
              launched.
            </p>
          </div>
          <div className="w-full sm:w-1/3 flex flex-col items-center text-center">
            ...
            <p className="text-base sm:text-lg">
              We get data from reliable sources such as Scopus, ORCID and
              OpenAlex.
            </p>
          </div>
          <div className="w-full sm:w-1/3 flex flex-col items-center text-center">
            ...
            <p className="text-base sm:text-lg">
              We only display the latest data to you so that you have the most
      ...
      <div className="bg-[#F3F4F6] w-full h-max text-[#000054] text-center py-10 sm:py-20 px-4">
        <h2 className="text-2xl sm:text-3xl mb-10 sm:mb-20 font-bold">
        ...
        <div className="w-full sm:w-5/6 lg:w-2/3 h-max mx-auto mb-10 flex flex-col text-justify">
          <Accordion>
            ...
            >
              <Typography component="span">
                Is this system completely foolproof?
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              Unfortunately, our system is not without error. For instance let's
              look at our CV Verification feature: we rely on Scopus, ORCID and
              OpenAlex (and Google Scholar as a last resort) to give you the
              verification results of candidates’ CVs, but what if the
              information that we want to verify falls outside of all these
              databases?, which means there will false negative cases.
              Therefore, we recommend that you use this system as a{" "}
              <b>supplement</b> to your own research, rather than a replacement.
            </AccordionDetails>
          </Accordion>
          <Accordion>
            <AccordionSummary
            ...
            >
              <Typography component="span">
                What type of statements in a CV can this system verify?
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              Considering the complexity of the tasks and the limitation of our
              abilities, we currently only verify the <b>Publications</b>{" "}
              section of a CV. Specifically, we check whether the publications
              listed in the CV can be found in Scopus, ORCID or OpenAlex. If
              they are not found in any of these databases, we will mark them as
              unverifiable.
            </AccordionDetails>
          </Accordion>
          <Accordion>
            ...
            >
              <Typography component="span">
                Does this system include any paid features?
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
          ...
          <Accordion>
            ...
            >
              <Typography component="span">
                Is there a limit on how many CVs I can upload in a day?
              </Typography>
            </AccordionSummary>
            <AccordionDetails>
              No, you can upload as many CVs as you want in a day. However, we
              need to emphasize that CVs containing more pages take longer to
              process. We estimate that a CV with 18 pages may take around 2
              minutes to process.
            </AccordionDetails>
          </Accordion>
        </div>
